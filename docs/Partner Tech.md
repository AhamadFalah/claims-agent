...
| **n8n** | Orchestrates the autonomous loop / triggers on new record. | Sign up; apply the event community code (see Discord). |
...
| **Minima / Mubit** | Route LLM calls to the cheapest model that clears the bar. | Key `mbt_…` from console.mubit.ai; redeem the event credit code (see Discord). |
